...
public class AllDessertMenusGUI extends JPanel{
	int page = 0;
	int[] locationX = { 20, 250, 20, 250, 20, 250 };
	int[] locationY = { 70, 70, 270, 270, 470, 470 };

	public AllDessertMenusGUI() {
		paintGUI();
	}
	...
	public void paintGUI() {
		...
		// 상단바
		JLabel topBar = new JLabel("메뉴 목록 - 디저트");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!manner.contentEquals("정렬")) {
					if(manner.contentEquals("별점높은순"))
						StartGUI.cafe.sortDessertStar();
					else if(manner.contentEquals("이름오름차순"))
						StartGUI.cafe.sortDessertNameAsc();
					else if(manner.contentEquals("이름내림차순"))
						StartGUI.cafe.sortDessertNameDes();
					else if(manner.contentEquals("판매량순"))
						StartGUI.cafe.sortDessertSale();
					removeAll();
					revalidate();
					repaint();
		...
		drinkButton.setBounds(370, 700, 90, 30);
		...
		NextButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if ((StartGUI.cafe.menudessertMgr.mList.size() / 6) > page) {
					page += 1;
					removeAll();
					revalidate();
					repaint();
		...
		DessertMenuGUI[] menuItems = new DessertMenuGUI[6]; // null로 초기화 후

		for (int i = 0; i < 6; i++) {
			int index = page * 6 + i;
			if (index < StartGUI.cafe.menudessertMgr.mList.size()) {
				menuItems[i] = new DessertMenuGUI(page * 6 + i); // 객체 생성
